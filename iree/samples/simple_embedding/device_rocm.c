...
// A example of setting up the the rocm driver.
...
#include "iree/samples/simple_embedding/simple_embedding_test_bytecode_module_rocm_c.h"

iree_status_t create_sample_device(iree_hal_device_t** device) {
  // Only register the rocm HAL driver.
  IREE_RETURN_IF_ERROR(
      iree_hal_rocm_driver_module_register(iree_hal_driver_registry_default()));
...
const iree_const_byte_span_t load_bytecode_module_data() {
  const struct iree_file_toc_t* module_file_toc =
      iree_samples_simple_embedding_test_module_rocm_create();
