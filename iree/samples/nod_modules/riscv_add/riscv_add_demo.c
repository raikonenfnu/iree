...
#include "iree/samples/nod_modules/riscv_add/riscv_add_c.h"
...
#include "iree/samples/nod_modules/riscv_add/riscv_add.h"
...
iree_status_t create_device_with_static_loader(iree_hal_device_t** device) {
  ...
  const iree_hal_executable_library_header_t** static_library =
      _forward_dispatch_0_library_query(
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_sync_device_create(iree_make_cstring_view("dylib-sync"), &params,
