...
#ifdef USE_NOD
#include "iree/samples/nlpnet/nlpnet_nod_bytecode_module_c.h"
#else
...
iree_status_t Run() {
  IREE_RETURN_IF_ERROR(iree_hal_dylib_sync_driver_module_register(iree_hal_driver_registry_default()));
...
#ifdef USE_NOD
  const struct iree_file_toc_t* module_file_toc = nlpnet_nod_bytecode_module_c_create();
#else
  ...
  iree_vm_module_t* modules[] = {hal_module, native_module, bytecode_module};
  ...
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
      arg1_buffer, shape2, IREE_ARRAYSIZE(shape2), IREE_HAL_ELEMENT_TYPE_SINT_32,
