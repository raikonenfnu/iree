...
iree_status_t Run() {
  ...
  // erased buffers.
  const char kMainFunctionName[] = "module.simple_bmm";
  ...
  const int kElementCount0 = 2*4*3;
  const int kElementCount1 = 2*3*6;
  ...
  IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
      ...
      IREE_HAL_BUFFER_USAGE_ALL, sizeof(float) * kElementCount0, &arg0_buffer));
  IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
      iree_hal_device_allocator(device), input_memory_type,
      IREE_HAL_BUFFER_USAGE_ALL, sizeof(float) * kElementCount1, &arg1_buffer));
  ...
  iree_hal_dim_t shape0[3] = {2,4,3};
  iree_hal_dim_t shape1[3] = {2,3,6};
  iree_hal_buffer_view_t* arg0_buffer_view = NULL;
  iree_hal_buffer_view_t* arg1_buffer_view = NULL;
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
      arg0_buffer, shape0, IREE_ARRAYSIZE(shape0), IREE_HAL_ELEMENT_TYPE_FLOAT_32,
      &arg0_buffer_view));
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
      arg1_buffer, shape1, IREE_ARRAYSIZE(shape1), IREE_HAL_ELEMENT_TYPE_FLOAT_32,
      &arg1_buffer_view));
  ...
  for (int i = 0; i < mapped_memory.contents.data_length / sizeof(float); ++i) {
    if (((const float*)mapped_memory.contents.data)[i] != 24.0f) {
...
int main() {
  ...
  fprintf(stdout, "simple_bmm passed\n");
  return ret;
}
