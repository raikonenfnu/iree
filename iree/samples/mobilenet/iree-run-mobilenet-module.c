...
// from mobilenet.mlir on the dylib-llvm-aot backend.
...
#include "iree/samples/mobilenet/mobilenet_bytecode_module_c.h"
...
iree_status_t Run(const iree_string_view_t image_path) {
  ...
  const struct iree_file_toc_t* module_file_toc = mobilenet_bytecode_module_c_create();
...
// // Place holder end
  ...
  // Read back the results. The output of the mobilenet model is a 1x10 prediction
...
int main(int argc, char** argv) {
  if (argc > 2) {
    fprintf(stderr, "Usage: iree-run-mobilenet-module <image file>\n");
    return -1;
  }
  iree_string_view_t image_path;
  if (argc == 1) {
    image_path = iree_make_cstring_view("test.png");
  } else {
