...
typedef struct {
  ...
  void* current_descriptor[IREE_HAL_ROCM_MAX_KERNEL_ARG];
  hipDeviceptr_t* device_ptrs[IREE_HAL_ROCM_MAX_KERNEL_ARG];
...
iree_hal_rocm_stream_command_buffer_cast(
    iree_hal_command_buffer_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_rocm_stream_command_buffer_vtable);
  return (iree_hal_rocm_stream_command_buffer_t*)base_value;
}

iree_status_t iree_hal_rocm_stream_command_buffer_create(
  ...
  *out_command_buffer = NULL;
  ...
  iree_hal_rocm_stream_command_buffer_t* command_buffer = NULL;
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_command_buffer_initialize(
        ...
        binding_capacity, &iree_hal_rocm_stream_command_buffer_vtable,
        &command_buffer->base);
...
static void iree_hal_rocm_stream_command_buffer_destroy(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_rocm_stream_command_buffer_t* command_buffer =
      iree_hal_rocm_stream_command_buffer_cast(base_command_buffer);
  IREE_TRACE_ZONE_BEGIN(z0);
...
bool iree_hal_rocm_stream_command_buffer_isa(
...
static iree_status_t iree_hal_rocm_stream_command_buffer_flush_collectives(
    iree_hal_rocm_stream_command_buffer_t* command_buffer) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "Collectives not implemented on ROCM");
}

static iree_status_t iree_hal_rocm_stream_command_buffer_begin(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_rocm_stream_command_buffer_t* command_buffer =
      iree_hal_rocm_stream_command_buffer_cast(base_command_buffer);
  (void)command_buffer;
  ...
  IREE_ROCM_TRACE_ZONE_BEGIN_EXTERNAL(
      ...
      /*file_name=*/NULL, 0,
      /*line=*/0, /*func_name=*/NULL, 0, "iree_hal_rocm_stream_command_buffer",
      strlen("iree_hal_rocm_stream_command_buffer"));

  return iree_ok_status();
}

static iree_status_t iree_hal_rocm_stream_command_buffer_end(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_rocm_stream_command_buffer_t* command_buffer =
      iree_hal_rocm_stream_command_buffer_cast(base_command_buffer);

//   IREE_RETURN_IF_ERROR(
//       iree_hal_rocm_stream_command_buffer_flush_collectives(command_buffer));
...
static void iree_hal_rocm_stream_command_buffer_begin_debug_group(
...
static void iree_hal_rocm_stream_command_buffer_end_debug_group(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_rocm_stream_command_buffer_t* command_buffer =
      iree_hal_rocm_stream_command_buffer_cast(base_command_buffer);
  (void)command_buffer;
...
static iree_status_t iree_hal_rocm_stream_command_buffer_execution_barrier(
    iree_hal_command_buffer_t* base_command_buffer,
...
//   IREE_RETURN_IF_ERROR(
//       iree_hal_rocm_stream_command_buffer_flush_collectives(command_buffer));
  // TODO(raikonen): implement ROCM barrier
  return iree_ok_status();
}

static iree_status_t iree_hal_rocm_stream_command_buffer_signal_event(
...
static iree_status_t iree_hal_rocm_stream_command_buffer_reset_event(
    iree_hal_command_buffer_t* base_command_buffer, iree_hal_event_t* event,
    iree_hal_execution_stage_t source_stage_mask) {
//   iree_hal_rocm_stream_command_buffer_t* command_buffer =
//       iree_hal_rocm_stream_command_buffer_cast(base_command_buffer);
...
static iree_status_t iree_hal_rocm_stream_command_buffer_wait_events(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_rocm_stream_command_buffer_discard_buffer(
    iree_hal_command_buffer_t* base_command_buffer, iree_hal_buffer_t* buffer) {
  // We could mark the memory as invalidated so that if managed ROCM does not
  // try to copy it back to the host.
  return iree_ok_status();
}

static iree_status_t iree_hal_rocm_stream_command_buffer_fill_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_rocm_stream_command_buffer_update_buffer(
...
//       iree_hal_rocm_stream_command_buffer_flush_collectives(command_buffer));
  ...
  hipDeviceptr_t target_device_buffer = iree_hal_rocm_buffer_device_pointer(
      iree_hal_buffer_allocated_buffer(target_buffer));
  hipDeviceptr_t dst = (hipDeviceptr_t)((uintptr_t)target_device_buffer +
                    iree_hal_buffer_byte_offset(target_buffer) + target_offset);
  ROCM_RETURN_IF_ERROR(
      command_buffer->context->syms,
      hipMemcpyHtoDAsync(dst, (void*)src, length, command_buffer->stream),
      "hipMemcpyHtoDAsync");

  return iree_ok_status();
}

static iree_status_t iree_hal_rocm_stream_command_buffer_copy_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
...
//       iree_hal_rocm_stream_command_buffer_flush_collectives(command_buffer));
  ...
  hipDeviceptr_t dst = (hipDeviceptr_t)((uintptr_t)target_device_buffer + target_offset);
  hipDeviceptr_t src = (hipDeviceptr_t)((uintptr_t)source_device_buffer + source_offset);
  ROCM_RETURN_IF_ERROR(command_buffer->context->syms,
                       hipMemcpyAsync(dst, src, length, hipMemcpyDeviceToDevice, command_buffer->stream),
                       "hipMemcpyAsync");

  return iree_ok_status();
}

static iree_status_t iree_hal_rocm_stream_command_buffer_collective(
...
static iree_status_t iree_hal_rocm_stream_command_buffer_push_constants(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_rocm_stream_command_buffer_push_descriptor_set(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_rocm_stream_command_buffer_dispatch(
    iree_hal_command_buffer_t* base_command_buffer,
...
//       iree_hal_rocm_stream_command_buffer_flush_collectives(command_buffer));
  ...
  IREE_ROCM_TRACE_ZONE_BEGIN_EXTERNAL(
      command_buffer->tracing_context, command_buffer->stream, kernel_params.function_name.data,
...
static iree_status_t iree_hal_rocm_stream_command_buffer_dispatch_indirect(
    iree_hal_command_buffer_t* base_command_buffer,
    iree_hal_executable_t* executable, int32_t entry_point,
    iree_hal_buffer_t* workgroups_buffer,
    iree_device_size_t workgroups_offset) {
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                          "need rocm implementation of dispatch indirect");
}

static iree_status_t iree_hal_rocm_stream_command_buffer_execute_commands(
    iree_hal_command_buffer_t* base_command_buffer,
...
static const iree_hal_command_buffer_vtable_t
    iree_hal_rocm_stream_command_buffer_vtable = {
        ...
        .begin = iree_hal_rocm_stream_command_buffer_begin,
        .end = iree_hal_rocm_stream_command_buffer_end,
        .begin_debug_group =
            iree_hal_rocm_stream_command_buffer_begin_debug_group,
        .end_debug_group = iree_hal_rocm_stream_command_buffer_end_debug_group,
        .execution_barrier =
            iree_hal_rocm_stream_command_buffer_execution_barrier,
        .signal_event = iree_hal_rocm_stream_command_buffer_signal_event,
        .reset_event = iree_hal_rocm_stream_command_buffer_reset_event,
        .wait_events = iree_hal_rocm_stream_command_buffer_wait_events,
        .discard_buffer = iree_hal_rocm_stream_command_buffer_discard_buffer,
        .fill_buffer = iree_hal_rocm_stream_command_buffer_fill_buffer,
        .update_buffer = iree_hal_rocm_stream_command_buffer_update_buffer,
        .copy_buffer = iree_hal_rocm_stream_command_buffer_copy_buffer,
        .collective = iree_hal_rocm_stream_command_buffer_collective,
        .push_constants = iree_hal_rocm_stream_command_buffer_push_constants,
        .push_descriptor_set =
            iree_hal_rocm_stream_command_buffer_push_descriptor_set,
        .dispatch = iree_hal_rocm_stream_command_buffer_dispatch,
        .dispatch_indirect =
            iree_hal_rocm_stream_command_buffer_dispatch_indirect,
        .execute_commands =
            iree_hal_rocm_stream_command_buffer_execute_commands,
};
