...
# Upcasts types that requires input to be from another type.
def cast_argtype_if_required(t: MatrixElemTypeId):
    if t == MatrixElemTypeId.F8E4M3FNUZ:
        return MatrixElemTypeId.F32
    return t


# Upcasts types that requires input to be from another type.
def get_castback_from_arg_op(t: MatrixElemTypeId):
    if t == MatrixElemTypeId.F8E4M3FNUZ:
        return "arith.truncf"
