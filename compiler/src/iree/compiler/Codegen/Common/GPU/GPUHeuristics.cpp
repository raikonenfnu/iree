...
bool isValidSchedule(const GPUMatmulShapeType &problem,
  ...
  // TODO: Add constraint on K-dim since, vector.transfer_read
  // would also need to be distribued by K-dim for A-matrix.
  ...
  bool isValidTileN = (nTileSize / elemsPerThread) % wgThreads == 0 ||
                      wgThreads % (nTileSize / elemsPerThread) == 0;

  return isValidN && isValidM && isValidK && isValidTileN;
}
