...
#define DEBUG_TYPE "iree-codegen-gpu-vectorize-load-store"
...
class GPUVectorizeLoadStorePass final
    : public GPUVectorizeLoadStoreBase<GPUVectorizeLoadStorePass> {
...
void GPUVectorizeLoadStorePass::runOnOperation() {
...
std::unique_ptr<OperationPass<ModuleOp>> createGPUVectorizeLoadStore() {
  return std::make_unique<GPUVectorizeLoadStorePass>();
}
