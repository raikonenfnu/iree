...
#define GEN_PASS_DEF_PADATTENTIONPASS
...
struct PadAttentionPass final : impl::PadAttentionPassBase<PadAttentionPass> {
  using impl::PadAttentionPassBase<PadAttentionPass>::PadAttentionPassBase;
...
padAttention(IREE::LinalgExt::AttentionOp attnOp,
  ...
  // Pad mask-tensor if any of its' dims needs padding.
  std::optional<Value> mask = attnOp.getMask();
  if (mask.has_value() && (!isConstantIntValue(padValues[batchIdx], 0) ||
                           !isConstantIntValue(padValues[mIdx], 0))) {
    mask = getPaddedValue(rewriter, loc, mask.value(),
                          {padValues[batchIdx], padValues[mIdx], zero});
  }
  ...
  auto paddedAttnOp = rewriter.create<IREE::LinalgExt::AttentionOp>(
      loc, TypeRange{paddedAcc.getType()}, paddedQuery, paddedKey, paddedValue,
      scale, ValueRange{paddedAcc},
      rewriter.getAffineMapArrayAttr(attnOp.getIndexingMapsArray()), mask);
