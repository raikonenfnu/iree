...
class GeneralizeTargetNamedOpPattern final
 ...
 public:
  ...
  LogicalResult matchAndRewrite(LinalgOpType linalgOp,
    ...
    FailureOr<linalg::GenericOp> genericOp = linalg::generalizeNamedOp(rewriter, linalgOp);
...
class FuseMatmulAndTranspose final
 ...
 public:
  ...
  // Inspo: https://github.com/llvm/llvm-project/blob/4f1c12425179608298dc39f5524ba2612609b5e4/mlir/lib/Dialect/Utils/StructuredOpsUtils.cpp
  LogicalResult matchAndRewrite(linalg::GenericOp linalgOp,
                                PatternRewriter &rewriter) const override {
    const unsigned rhsId = 1;
    if(!isMatmulOrBatchMatmul(linalgOp)) return failure();
    Value rhs = linalgOp.getInputOperand(rhsId)->get();
    ...
    assert(rhsMap.getNumResults() == rank);
    SmallVector<AffineExpr, 3> exprs;
    for (auto dim_id : perm) {
      exprs.push_back(rhsMap.getResult(dim_id));
    }
    AffineMap transposedRhsMap = AffineMap::get(rhsMap.getNumDims(), 0, exprs, getContext());
    ...
    SmallVector<AffineMap> newIndexingMaps = {lhsMap, transposedRhsMap, accMap};

    // Generate new list of args.
    Value newRhs = transposeOp.getInputOperand(0)->get();
    Value lhs = linalgOp.getInputOperand(0)->get();
    Value acc = linalgOp.getOutputOperand(0)->get();
    SmallVector<Value> inputs = {lhs, newRhs};

    // Generate a new genericOp.
    SmallVector<StringRef> loopTypes;
    ArrayAttr loopAttributes = linalgOp.getIteratorTypes();
    for (auto it : loopAttributes) {
      loopTypes.push_back(it.cast<StringAttr>().getValue());
    }
    linalg::GenericOp genericOp = rewriter.create<linalg::GenericOp>(
        linalgOp.getLoc(), linalgOp.getResultTypes(), /*inputs*/ inputs, /*outputs*/ acc, newIndexingMaps, loopTypes);
      // Block consumerBlock = linalgOp->getRegion(0).front();
      // genericOp.getRegion().push_back(consumerBlock);
    // llvm::outs()<<"new op regions:"<<genericOp.getOperation()->getNumRegions()<<"\n";
    // llvm::outs()<<"new op regions:"<<genericOp.getOperation()->getNumRegions()<<"\n";
    // llvm::outs()<<"new op blocks:"<<genericOp.getRegion().getBlocks().size()<<"\n";
    // llvm::outs()<<"old op regions:"<<linalgOp.getOperation()->getNumRegions()<<"\n";
    // llvm::outs()<<"old op blocks:"<<linalgOp.getRegion().getBlocks().size()<<"\n";
