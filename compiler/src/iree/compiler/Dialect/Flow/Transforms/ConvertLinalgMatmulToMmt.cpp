...
class LinalgMatmulOpToLinalgMmtPattern final
 ...
 public:
  ...
  LogicalResult matchAndRewrite(linalg::MatmulOp matmulOp,
    ...
    Value transposedRhs = rewriter.create<tensor::EmptyOp>(
      loc, transposedRhsShape, rhsElemType);
    SmallVector<StringRef> loopAttributeTypes(nloops,
                                            getParallelIteratorTypeName());


    Value packedRhs = rewriter.create<linalg::GenericOp>(
    ...
    //TransposeOp
    Value initOp = rewriter.create<tensor::EmptyOp>(
      loc, rhsShape, rhsElemType);
    SmallVector<int64_t, 2> transposedPerm = {1, 0};
    Value transposePackedRhs = rewriter.create<linalg::TransposeOp>(loc, packedRhs, initOp, transposedPerm).getResults()[0];

    //MatmulOp
    Value packedMatmul = rewriter.create<linalg::MatmulOp>(
      loc, matmulOp.getResult(0).getType(),
      ArrayRef<Value>{lhs, transposePackedRhs},
      ArrayRef<Value>{acc}).getResult(0);
