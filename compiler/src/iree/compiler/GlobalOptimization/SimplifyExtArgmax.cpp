...
class SimplifyExtArgmaxFuncPattern final
...
public:
  ...
  LogicalResult matchAndRewrite(func::CallOp op,
    ...
    // TODO: Check for callee name.
    if (op.getOperands().size() != 1)
      return failure();

    auto expandLastLogitsOp =
        dyn_cast<tensor::ExpandShapeOp>(op.getOperand(0).getDefiningOp());
    if (!expandLastLogitsOp)
      return failure();

    auto extractSliceOp = dyn_cast<tensor::ExtractSliceOp>(
        expandLastLogitsOp.getSrc().getDefiningOp());
    if (!extractSliceOp)
      return failure();

    auto expandAllLogitsOp = dyn_cast<tensor::ExpandShapeOp>(
        extractSliceOp.getSource().getDefiningOp());
    if (!expandAllLogitsOp)
      return failure();
    auto expandSrcType =
        dyn_cast<TensorType>(expandAllLogitsOp.getSrc().getType());
    if (!expandSrcType)
      return failure();
    auto expandSrcShape = expandSrcType.getShape();
    auto expandDstType =
        dyn_cast<TensorType>(expandAllLogitsOp.getResult().getType());
    if (!expandDstType)
      return failure();
    ...
    auto extGenericOp =
        dyn_cast<linalg::GenericOp>(expandAllLogitsOp.getSrc().getDefiningOp());
    if (!extGenericOp)
      return failure();
    ...
    // Create extract slice on source of extfop without dropped dim.
    rewriter.setInsertionPoint(expandLastLogitsOp);
    auto loc = op.getLoc();
    ...
    auto newExtFOp =
        rewriter
            ...
            .getResult(0);
    // replace dpsInit of .
    rewriter.startOpModification(expandLastLogitsOp);
    expandLastLogitsOp.setOperand(newExtFOp);
    rewriter.finalizeOpModification(expandLastLogitsOp);
    return success();
  }
};
...
void SimplifyExtArgmaxPass::runOnOperation() {
  ...
  patterns.insert<SimplifyExtArgmaxPattern, SimplifyExtArgmaxFuncPattern>(
      context);
  if (failed(
