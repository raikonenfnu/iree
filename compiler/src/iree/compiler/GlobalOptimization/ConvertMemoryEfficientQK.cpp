...
public:
  ...
  LogicalResult matchAndRewrite(linalg::BatchMatmulTransposeBOp bmmtOp,
    ...
    int64_t numHeads = bbmtLhsShape[0];
    int64_t hiddenDim = bbmtLhsShape[2];
    ...
    auto resultQKcacheType = RankedTensorType::get(
        QKcacheStaticSizes, bbmtResultType.getElementType());
    Value QKCacheAcc = rewriter.create<tensor::EmptyOp>(loc, resultQKcacheType,
                                                        QKcacheDynSizes);
    Value zero = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getZeroAttr(bbmtResultType.getElementType()));
    ...
    auto resultQNewKSliceType = RankedTensorType::get(
        {numHeads, bbmtLhsShape[1], newKSliceStaticSize[2]},
        bbmtResultType.getElementType());
    Value QNewKSliceAcc =
...
void ConvertMemoryEfficientQKPass::runOnOperation() {
  ...
  patterns.insert<ConvertMemoryEfficientQKPattern>(context);
  if (failed(
